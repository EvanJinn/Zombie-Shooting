...
public class Hud : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (curMenuAlpha < .01f && transiting && transform.Find("Menu").gameObject.activeSelf)
        {
            transiting = false;
        }

        // choose item
        if (transform.Find("Menu").gameObject.activeSelf && curMenuAlpha > .88f)
        {
#if !MOBILE_INPUT
    ...
    private void ChangeAlphaOfMenu()
    {
        ...
        Color color = transform.Find("Menu").Find("BackGround").GetComponent<Image>().color;
        transform.Find("Menu").Find("BackGround").GetComponent<Image>().color = new Color(color.r, color.g, color.b, curMenuAlpha);
        foreach (GameObject menuGo in itemMenuGos)
        {
            ...
            color = menuGo.transform.Find("Text").GetComponent<Text>().color;
            menuGo.transform.Find("Text").GetComponent<Text>().color = new Color(color.r, color.g, color.b, curMenuAlpha);
        }
    }
    ...
    private void ManageMenuOnStartup()
    {
        ...
        foreach (GameObject goItem in itemMenuGos)
        {
            if (i < plAtts.Guns.Count && plAtts.Guns[i])
            {
                ...
                goItem.transform.Find("Text").GetComponent<Text>().text = plAtts.Guns[i].GetComponent<GunAtt>().weaponName;
            ...
            else
            {
                ...
                goItem.transform.Find("Text").GetComponent<Text>().text = "";
            }
            i++;
        }
    }
    ...
    public int AddGoToMenu(GameObject goToAdd) // goToAdd needs to have GunAtt script
    {
        foreach (GameObject goItem in itemMenuGos)
        {
            if (goItem.GetComponent<MenuItemHolder>().go == null)
            {
                ...
                goItem.transform.Find("Text").GetComponent<Text>().text = gunAtt.weaponName;
                return 0;
            }
        }
